# Задача 34:  Винни-Пух попросил Вас посмотреть, есть
# ли в его стихах ритм. Поскольку разобраться в его
# кричалках не настолько просто, насколько легко он
# их придумывает, Вам стоит написать программу. Винни-Пух
...
# Примечание: бинарной операцией называется любая
# операция, у которой ровно два аргумента, как, например,
...
def print_operation_table(operation, num_rows=6, num_columns=6):
    for i in range(1,num_rows+1):
        ...
        for j in range(1,num_columns+1):
            s.append(operation(i,j))
        print(*[str(x) for x in s])
print(print_operation_table(lambda x,y:x*y))

#во второй задаче не уверен, что сделал правильно. Смысл задачи не очень ясен. Возможно сделал не по задумке именно в решении
